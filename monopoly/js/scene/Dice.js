...
(function (global) {
    ...
    const DIE_SIZE = (typeof window !== 'undefined' && window.DICE_SIZE) || 1.0;   // edge length
    ...
    function makeFaceTexture(value) {
        ...
        // Плоская белая грань — без градиента
        ...
        for (const key of LAYOUTS[value]) {
            ...
            // Точка: плоский чёрный круг, без тени и блика
            ctx.beginPath();
    ...
    /** Куб со скруглёнными рёбрами и углами.
        Плоская часть грани НЕ трогается (текстура с точками остаётся ровной),
        сдвигаются только вершины в узкой полосе у рёбер — там и получается
        скругление. Чем больше сегментов, тем оно глаже. 6 material-групп
        BoxGeometry сохраняются, поэтому грани не перепутываются. */
    function makeRoundedBoxGeometry(size, radius, seg) {
        try {
            const g = new THREE.BoxGeometry(size, size, size, seg, seg, seg);
            const pos = g.attributes.position, nrm = g.attributes.normal;
            const half = size / 2 - radius;
            const v = new THREE.Vector3(), c = new THREE.Vector3(), dvec = new THREE.Vector3();
            for (let i = 0; i < pos.count; i++) {
                v.fromBufferAttribute(pos, i);
                c.set(
                    Math.max(-half, Math.min(half, v.x)),
                    Math.max(-half, Math.min(half, v.y)),
                    Math.max(-half, Math.min(half, v.z)));
                dvec.subVectors(v, c);
                if (dvec.lengthSq() > 1e-10) {
                    dvec.normalize();
                    v.copy(c).addScaledVector(dvec, radius);
                    nrm.setXYZ(i, dvec.x, dvec.y, dvec.z);
                }
