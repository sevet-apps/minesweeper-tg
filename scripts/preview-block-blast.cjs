...
function page(url) {
    ...
    return `<!doctype html><html lang="${url.searchParams.get('lang') || 'ru'}" data-theme="${url.searchParams.get('light') ? 'light' : 'dark'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">${styles}<link rel="stylesheet" href="/assets/block-blast/visuals.css"><link rel="stylesheet" href="/assets/block-blast/picker.css"><script src="/assets/block-blast/motion.js"></script><script src="/assets/block-blast/picker.js"></script><script src="/assets/block-blast/visuals.js"></script><style>.qa-tools{position:fixed;bottom:8px;left:8px;right:8px;z-index:11000;display:flex;gap:5px;flex-wrap:wrap;justify-content:center;font:11px sans-serif}.qa-tools button{background:#263746;color:#ddd;border:1px solid #456;padding:7px;border-radius:8px}.qa-tools output{width:100%;text-align:center;color:#99acba;pointer-events:none}body{overflow:hidden}</style></head><body class="${url.searchParams.get('touch') ? '' : 'desktop'}">${markup}<div class="qa-tools"><button onclick="setup()">Сцена</button><button onclick="demo('row')">Линия</button><button onclick="demo('cross')">Пересечение</button><button onclick="demo('all')">Всё поле</button><button onclick="stress()">Нагрузка</button><button onclick="lift()">Поднять</button><output id="qaStatus">Локальная проверка · без сохранения</output></div><script>
...
new MutationObserver(records=>{for(const record of records){if(record.attributeName==='hidden'&&record.target.matches('.bb-picker-overlay')&&!record.target.hidden){const values=[],start=performance.now(),panel=record.target.querySelector('.bb-picker-panel');function sample(){values.push(panel.getBoundingClientRect().top);if(performance.now()-start<480)requestAnimationFrame(sample);else document.documentElement.dataset.sheetTravel=JSON.stringify({first:Math.round(values[0]),last:Math.round(values.at(-1)),wrongWay:values.slice(1).filter((v,i)=>v>values[i]+1).length})}requestAnimationFrame(sample)}}}).observe(document.body,{subtree:true,attributes:true,attributeFilter:['hidden']});
</script></body></html>`;
}
