...
(function (global) {
    ...
    class SceneManager {
        constructor(container) {
            ...
            /* Арена по видимой области контейнера: стены ровно на его границах.
               При y=10 и fov=42° видимая высота ≈ 2·tan(21°)·10 ≈ 7.67. */
            const visD = 2 * Math.tan(42 / 2 * Math.PI / 180) * 10;
            const visW = visD * (this.width / this.height);
            this._arena = {
                width:  Math.max(4, visW - 0.35),
                depth:  Math.max(4, visD - 0.35),
                height: ARENA.height,
                floorY: ARENA.floorY,
            };
        ...
        _addArenaFloorAndWalls(world) {
            ...
            const A = this._arena || ARENA;
            floorBody.position.y = A.floorY;
            world.addBody(floorBody);

            const t = 0.4, h = A.height;
            const w = A.width, d = A.depth;
            const y = A.floorY + h/2;
            const walls = [
        ...
        get arena()     { return this._arena || ARENA; }
