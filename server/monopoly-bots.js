...
function act(g, ph) {
    ...
    switch (ph.phase) {
        ...
        case 'await-pay':
            /* Платит, если хватает наличных. Если нет — не сдаётся сразу, а
               идёт обычной процедурой: сервер сам закладывает поля и продаёт
               филиалы, и только при нехватке всего этого объявляет банкротство.
               Раньше бот банкротился с полными карманами имущества. */
            return later(pause(800, 600), () =>
                p.money >= ph.amount ? g.pay(id) : g.forceResolvePay());
